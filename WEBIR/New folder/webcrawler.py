...
def url_nor(extracted_links):
    ...
    domain = '.ku.ac.th'
    not_be_there=['.arj','.asf','.asx','.asp','.au','.avi','.bmp','.cab','.cdf'
...
def path_store(url,raw):
    ...
    if len(result.path) < 150: 
        ...
        #query site 
            ...
            print('have  2 :')
        #elif 'bachelor-degree' in url:
            #print('fuck off this url')
        #elif 'homeclub' in url:
            #print('fuck off this url')
        #non html
        else:
...
#--- main process ---#
# e = error get_page / l = long name cant mkdir
while len(visited_q) < 10000 + e + l:
    current_url = dequeue()
